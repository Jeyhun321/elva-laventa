-- Elva LaVenta — заказ доступен ЛЮБОМУ вошедшему (Google или e-mail).
-- Supabase → SQL Editor → New query → вставьте этот файл целиком → Run.
--
-- Единственное отличие от supabase/fix-telegram-order-notify.sql:
-- убрана проверка `provider = 'google'` (ошибка GOOGLE_AUTH_REQUIRED).
-- Сессия по-прежнему обязательна: auth.uid() is null → AUTH_REQUIRED.
-- Серверная цена, проверка размеров, order_items, Telegram-уведомление
-- и очистка корзины — БЕЗ ИЗМЕНЕНИЙ.
