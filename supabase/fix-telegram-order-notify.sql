-- Elva LaVenta — Telegram уведомление о новом заказе.
-- Supabase → SQL Editor → New query → вставьте этот файл целиком → Run.
-- Скрипт сохраняет текущую защищённую логику оформления заказа:
-- авторизация Google, проверка полей, размеров и очистка корзины остаются без изменений.
-- Если Telegram временно недоступен, заказ всё равно успешно создаётся.
