...
**Password reset — root cause доставки найден (LAV-BUG-059): проблема в Supabase Email provider, НЕ в коде.** LIVE-проба на production: `resetPasswordForEmail(dummy, redirectTo=.../elva-laventa/reset)` → **error:null** — API принимает запрос, redirect/код корректны. Письмо не доходит из-за встроенного Supabase email-провайдера (rate-limit/не для прод/спам). **OWNER ACTION:** включить **Custom SMTP** (Dashboard → Auth → SMTP; credentials только на сервере) + проверить Site URL / Redirect URLs (`.../elva-laventa/reset`) + Auth Logs. Код: успех показывается только при реальном успехе API; добавлен аудит `PASSWORD_RESET_REQUESTED/FAILED` (маск. email, без токена). Inbox delivery подтверждает владелец после SMTP. См. BUGS #LAV-BUG-059.

**Admin header cleanup + password-reset robustness (Tasks 1–2). Task 3 (owner→real-user impersonation) — задизайнен, вынесен в отдельный тестируемый этап.**
- **Task 1 (готово):** на `/admin` в шапке скрыты storefront-контролы (UserMenu/профиль, Sevimlilər, Səbət) — они только для магазина. Витрина не тронута (favorites/cart есть). `src/components/Header.jsx` (условие `isAdminRoute`). Проверено: `/admin` → нет fav/cart/user-menu; `/` → есть.
- **Task 2 (код готово; доставка письма = owner-config):** `ResetPasswordPage` теперь устойчив ко ВСЕМ форматам recovery-ссылок Supabase: PKCE `?code=` (exchangeCodeForSession), `?token_hash=&type=recovery` (verifyOtp) и hash/implicit (`PASSWORD_RECOVERY` через detectSessionInUrl). Добавлен «проверяю ссылку» вместо ложного «просрочено». SPA-fallback GH Pages (`404.html=index.html`) сохраняет URL/query. Проверено: `/reset` без кода → корректный «link expired», 0 console errors. **OWNER ACTION (Supabase Dashboard → Authentication → URL Configuration):** Site URL = `https://jeyhun321.github.io/elva-laventa/`; в Redirect URLs добавить `https://jeyhun321.github.io/elva-laventa/reset` (и/или `.../elva-laventa/**`). Для надёжной доставки писем — настроить custom SMTP (встроенный Supabase-SMTP жёстко лимитирован, часто в «Спам»). Клиент шлёт `resetPasswordForEmail(redirectTo=.../elva-laventa/reset)` и показывает «Ссылка … отправлена на {email}» / понятную ошибку.
- **Task 3 — DONE ✅ (принято владельцем; live-проверено под owner-сессией 2026-08-21):** SQL применён; вручную подтверждено: `/admin`→OTP→Пользователи, у owner «Текущий аккаунт», у прочих «Войти как пользователь»; вход в User A показывает его реальные cart/favorites/profile; возврат в админку работает и owner-сессия сохраняется; после входа в другого пользователя данных предыдущего не остаётся; баннер появляется/исчезает; checkout в имперсонации заблокирован; обычная витрина вне режима работает. Server-side (anon 12/12 RPC denied, grant-таблица закрыта, service_role в бандле NONE) — подтверждено автоматически. Ниже — реализация как есть (НЕ менять без причины). «Войти как пользователь» — owner работает с РЕАЛЬНЫМИ данными выбранного пользователя. **Архитектура:** actor=owner (сессия не меняется, второй auth-клиент НЕ создаётся), effectiveAccount=target UUID. Доступ к данным target — ТОЛЬКО через is_admin-gated security-definer RPC (`supabase/admin-impersonation.sql`): grant-таблица с **TTL 45 мин** (`admin_impersonation_start/end`), read/write cart/favorites, read profile/wheel-reward/orders. Каждый RPC: `is_admin()` + активный grant. RLS обычных таблиц НЕ тронут; обычные юзеры остаются на `auth.uid()`. `service_role` во фронте нет (проверено — в бандле NONE). Аудит `USER_IMPERSONATION_STARTED/ENDED` в `system_logs`. **Frontend:** `ImpersonationContext` (sessionStorage tab-scoped + auto-expire), `ShopContext` роутит cart/favorites через imp-RPC на effectiveAccount (кэш чужого аккаунта НЕ персистится; изоляция A→B через сброс на смене effective), баннер «Вы вошли в аккаунт …» + «Вернуться в админку», UserMenu/checkout-prefill показывают target, купон checkout — target, **place_order в имперсонации ЗАБЛОКИРОВАН** (admin-сообщение; обычный checkout не тронут). UI: в Admin→Пользователи у не-owner «Войти как пользователь», у owner «Текущий аккаунт». **OWNER ACTION REQUIRED:** выполнить `supabase/admin-impersonation.sql`. **Проверено (без owner-OAuth):** build+`npm test` 21/21; баннер рендерится + exit чистит sessionStorage и уходит в /admin; imp-RPC вызываются (пока 404 — SQL не применён; ShopContext их гасит); гость — без impersonation-состояния; overflow нет. Живой e2e (owner→target real data→reversible write→A/B изоляция→security 403 для non-owner) — по manual-плану в отчёте, после применения SQL, под owner-сессией.

**(пред.) UI: полноэкранная 404 для non-owner /admin.** Маленькая карточка заменена на брендовую fullscreen-страницу `src/components/NotFoundPage.jsx` (position:fixed;inset:0;z-index:3000 — перекрывает storefront header/footer/tabbar): лого Elva LaVenta сверху-слева, гигантский serif «404» (--plum), «Not Found»/`nf_heading`, подтекст, pill-кнопка «На главную»→`/`, мягкие розовые лепестки (inline-SVG). i18n AZ/RU/EN (`nf_heading`/`nf_text`/`nf_home`). Security-UX: без «нет прав»/owner email/OTP/admin-данных — обычная 404, не раскрывает админку. Owner-флоу (OTP→панель) и вся auth/RLS/RPC логика НЕ тронуты. Verified: build+`npm test` 21/21; desktop 1440 и mobile 390 — без overflow, storefront layout перекрыт; guest /admin → login (реальный гейт); 0 console errors. Файлы: `NotFoundPage.jsx` (new), `AdminPage.jsx` (замена + удалён старый NotFoundScreen), `translations.js`, `index.css`.
