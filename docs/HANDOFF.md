...
**Admin auth упрощён — удалена вторая admin-OTP-система (один source of truth).** Гейт `/admin`: anon → обычный вход (Google/пароль); authenticated → фронт спрашивает сервер `rpc('is_admin')` → true → админка сразу (без OTP), false → 404. Удалены `EmailOtpScreen`/`AdminSetupRequired`/`ADMIN_OTP_EMAIL`/`isOwnerEmail`/`signInWithOtp`/`verifyOtp` и sessionStorage `elva-admin-otp-verified`; client-side admin-unlock флагов не осталось. Смена аккаунта → пересчёт is_admin → мгновенный unmount; возврат на owner → без OTP. build OK; `npm test` 21/21; guest `/admin` → login без OTP; anon `is_admin()`→false (сервер отклоняет). Owner-live (admin сразу без OTP) — за владельцем (OAuth). Изменён только `src/pages/AdminPage.jsx`.
