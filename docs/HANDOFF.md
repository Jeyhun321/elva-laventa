...
- **Task 3 (НЕ shipped — сознательно):** «Войти как пользователь» = оператор владельца работает с РЕАЛЬНЫМИ данными выбранного пользователя (профиль/заказы/корзина/избранное/купоны), включая записи и путь checkout. Безопасно это делается ТОЛЬКО через is_admin-gated security-definer RPC с target UUID (не ослабляя RLS, без service_role во фронте) + централизованный `effectiveAccount`-контекст + баннер + expiry + аудит. Это большой production-checkout-critical слой, а обязательные тесты требуют owner-OAuth + реальных тест-юзеров A/B (я их выполнить не могу). Пушить непротестированную имперсонацию в живой checkout нельзя (риск утечки данных между юзерами/поломки заказов). Дизайн — в отчёте; реализовывать отдельным этапом под тест владельца.
