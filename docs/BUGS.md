...
## LAV-BUG-058 — Admin owner hardening (is_admin() пинится к immutable owner UUID)
- **Module:** `public.is_admin()` + `profiles.role` + frontend-гейт (`src/pages/AdminPage.jsx`)
- **Platform:** both (серверная авторизация)
- **Environment:** Production
- **Priority:** P1 · **Severity:** S2 · **Status:** FIXED в коде+SQL — **требует применения владельцем** `supabase/fix-admin-owner.sql`
- **Found By:** Owner (report) · **Found Date:** 2026-08-21 · **Developer:** Claude Code
- **Description / история:** первоначально сообщено как «чужой аккаунт открывает /admin», при этом владелец был указан как `olegperov2002@gmail.com`. **Уточнение владельца (2026-08-21): единственный настоящий owner — `alekberov.ceyhun2002@gmail.com`** (инфо про olegperov было ошибочным). Т.е. фактической privilege-escalation не было — alekberov и есть владелец. Промежуточная миграция с olegperov в коммит НЕ попала как активная (email в активном коде и docs исправлен обратно на alekberov; иначе она заблокировала бы реального владельца).
- **Deliverable (полезное упрочнение):** `is_admin()` переведён с чистой email-проверки на **тройную**: `auth.uid() = OWNER_UUID` (immutable, вшит из auth.users) AND `profiles.role='admin'` AND `jwt email = alekberov.ceyhun2002@gmail.com`. Плюс миграция снимает `role='admin'` у всех прочих аккаунтов (на случай случайных админов) — пользователей не удаляет.
- **Fix Summary:** идемпотентная миграция `supabase/fix-admin-owner.sql`: (1) резолвит UUID `alekberov.ceyhun2002@gmail.com`; (2) снимает admin у всех кроме owner; (3) upsert `role='admin'` владельцу; (4) пересоздаёт `is_admin()` с вшитым owner UUID (+role+email). Frontend `ADMIN_OTP_EMAIL` = alekberov (UX-гейт; серверная is_admin — реальная граница). Старые `admin-lockdown.sql`/`admin-access-email.sql` помечены superseded (email = alekberov). Все admin-only RPC/RLS (admin_list_users, promo, wheel, products, orders, system logs) опираются на эту же is_admin().
- **Fix Verification checklist:**
  - [x] Frontend: `vite build` OK; `npm test` 21/21; в активном коде/бандле owner = `alekberov.ceyhun2002@gmail.com`, `olegperov` НЕ присутствует нигде (src/supabase/docs — clean).
  - [ ] **OWNER ACTION REQUIRED:** выполнить `supabase/fix-admin-owner.sql` в Supabase → SQL Editor. Затем LIVE: `select u.email,p.role from profiles p join auth.users u on u.id=p.id where p.role='admin'` → ровно одна строка `alekberov.ceyhun2002@gmail.com`; под alekberov is_admin()=true (/admin работает); под любым другим аккаунтом is_admin()=false (/admin → 404, admin RPC → AUTH_REQUIRED).
...
- **Status:** ОТКРЫТ — доставка email **нестабильна** (подтверждено Claude вживую 2026-08-22). Требует Custom SMTP. Кодовая часть корректна.
- **Found By:** Owner (report) · **Found Date:** 2026-08-21 · **Developer:** Claude Code
- **Update 2026-08-22 (важно — откат преждевременного RESOLVED):** ранее статус был помечен RESOLVED на основе owner-«pass», который оказался недостоверным (owner подтверждал не проверяя). При живой перепроверке Claude через admin-OTP (тот же встроенный email-провайдер Supabase, что и recovery): **первый OTP-код дошёл, второй OTP-код НЕ пришёл сразу** (пришёл третий после повторной отправки). Это ровно симптом root cause LAV-BUG-059 — встроенный провайдер доставляет нестабильно/с задержкой/rate-limit. Recovery-письмо и admin-OTP идут через ОДИН провайдер. Вывод: **доставка email реально нестабильна**, Custom SMTP всё ещё необходим. Полный флоу password recovery inbox остаётся НЕ подтверждённым (owner-manual после Custom SMTP).
