...
- **Second investigation (2026-08-20, после сообщения владельца, что симптом повторился на реальном устройстве уже после commit `e0efea1`):** проведено НОВОЕ независимое расследование (playwright-mobile). **Reproduced: НЕТ** в эмуляции. Проверено детерминированно: (a) стресс-тест **60 циклов** product↔back + visibility hidden/visible + offline/online → **навигация 60/60**, **0 net-утечек listener'ов** (patched add/removeEventListener), **0 новых setInterval** (нет таймер-leak); (b) все navigate-guards: `AccountHomeRedirect` (A→B, 056 стабилизировал accountId — null-скачка нет), `CheckoutPage` empty-cart (корзина не пустеет), `ProductPage` `!product` → показывает «не найдено» **без** navigate (маршрут не роняет), `useInactivityRedirect` → намеренный одноразовый редирект на `/` после ≥30мин (состояние сохраняется, не freeze); (c) CatalogContext `revalidate` — silent (loading не поднимает) + seq-guard, realtime-канал с cleanup → resume не блэнкует routed-страницу; (d) stale-chunk после редеплоя уже покрыт `recovery.js` (reload при chunk-error). **Второго code-level root cause в SPA не найдено.** Остаток — реально-девайсовый фактор (настоящий OS tab-suspension + авторизованная сессия + возможный редеплой + «первый тап будит вкладку»), который эмулятор не создаёт. **Действие (per задача §13):** усилена персистентная диагностика для точного разбора СЛЕДУЮЩЕГО реального инцидента — `src/lib/lifecycleDiag.js`: capture-phase `pointerdown` пишет `tap {onProductLink, topReachesCard, top, pointerEvents}` (различает «тап не дошёл / overlay / дошёл»), `route` на смену маршрута, `nav-redirect {reason}` для КАЖДОГО редиректа (account-switch / idle-30m / empty-cart), `catalog-apply {products}`, `shop-loaded {cart,fav}`; новый `window.__lavDiagDetailed()` — events + live DOM-снапшот (overlay над центром/карточкой, `.wheel-modal`/`.wheel-backdrop`, body overflow/pointer-events, horizontal overflow, productCount). Без токенов/секретов; консоль молчит без `localStorage.elva_diag='1'`. **Изменённые файлы (2-е расследование):** `src/lib/lifecycleDiag.js`, `src/App.jsx`, `src/hooks/useInactivityRedirect.js`, `src/context/CatalogContext.jsx`, `src/context/ShopContext.jsx`. Кода поведения auth/nav/cart НЕ менял (только инструментация). build+`npm test` 13/13, playwright 0 console errors. **Статус остаётся: на реальном устройстве НЕ подтверждён как исправленный** — при следующем сбое владелец присылает `window.__lavDiagDetailed()`; по timeline будет виден точный виновник (guard-reason / overlay / tap-not-reached / stale state).
- **HEALTHY BASELINE (2026-08-20, production, long-idle bug НЕ проявлялся) — эталон нормального поведения. Статус: HEALTHY BASELINE CAPTURED / BROKEN DEVICE SNAPSHOT PENDING.**
  - **Snapshot (`window.__lavDiagDetailed().snapshot`) — НОРМА:** `productHit.reaches=true`, `centrePointerEvents=auto`, `bodyPointerEvents=auto`, `bodyOverflow=visible`, `horizontalOverflow=false`, `wheelModal=false`, `wheelBackdrop=false`, `productCount>0` (каталог не пуст).
  - **Events — НОРМАЛЬНАЯ полная (ре)инициализация:** `diag-installed → auth{event:INITIAL_SESSION, hasSession:true, uid:X} → auth{event:INIT, hasSession:true, uid:X} → shop-sync{acc:X} → catalog-apply{products:15} → shop-loaded{acc:X, cart, fav}`, далее по действию `route → tap{onProductLink:true, topReachesCard:true} → route{to:/product/..}`. AUTH: `hasSession` всегда `true`, `uid` неизменен, НЕТ `SIGNED_OUT`, НЕТ `session=null`; если есть `auth-resolve` → `outcome:keep`.
  - **Что означает `diag-installed` + повторяющаяся последовательность:** `installLifecycleDiag()` (`src/main.jsx`) выполняется ОДИН раз за полное выполнение бандла (guard `installed` в module-scope). Значит `diag-installed` = маркер **полной cold-boot (ре)инициализации приложения**. Ring живёт в `sessionStorage` и переживает reload в пределах вкладки-сессии, поэтому НЕСКОЛЬКО `diag-installed` в одном timeline = вкладку **несколько раз полностью перезагрузило** (типично для mobile: OS discard backgrounded tab → reload на возврате; либо `recovery.js` reload при stale-chunk; либо ручной reload). Это НЕ BFCache-restore (при bfcache пришёл бы `pageshow{persisted:true}` БЕЗ нового `diag-installed`). Это lifecycle-факт, НЕ root cause. В healthy каждая реинициализация ЗАВЕРШАЕТСЯ полным набором (INITIAL_SESSION hasSession:true + catalog-apply 15 + shop-loaded).
  - **Подозрительного в healthy timeline само по себе НЕТ.** Единственная заметная деталь — `auth-resolve` может присутствовать даже в норме: значит Supabase кратко отдал null-сессию, 056-grace её отложил и `getSession` подтвердил `keep`. Это ОЖИДАЕМО (механизм 056 работает), НЕ баг. В BROKEN тот же `auth-resolve` с `outcome:clear` или `SIGNED_OUT`, очистивший user, будет красным флагом.
  - **COMPARISON CHECKLIST — HEALTHY vs BROKEN (детерминированный diff):**
    | Категория | Сигнал | HEALTHY | BROKEN red-flag |
    |---|---|---|---|
    | AUTH | `auth.event` | INITIAL_SESSION/INIT/SIGNED_IN/TOKEN_REFRESHED | `SIGNED_OUT` (не от logout), повтор SIGNED_OUT↔SIGNED_IN |
    | AUTH | `hasSession` | всегда `true` | `false` в момент/после resume |
    | AUTH | `uid` | неизменен (X→X) | X→null, X→другой, X→null→X с задержкой |
    | AUTH | `auth-resolve.outcome` | отсутствует или `keep` | `clear` |
    | ROUTING | `tap.onProductLink` | `true` | `false` (тап мимо ссылки) |
    | ROUTING | `tap.topReachesCard` | `true` | `false` (сверху overlay) |
    | ROUTING | `route.to` после tap | `/product/..` | нет `route` (navigate не сработал) |
    | ROUTING | `nav-redirect.reason` | отсутствует (или ожидаемый idle-30m) | `empty-cart` / `account-switch` сразу после tap = маршрут откатило |
    | ROUTING | порядок | tap → route(/product) | tap → route(/product) → route(/) (мгновенный откат) |
    | SHOP | `shop-loaded.acc` | = uid, присутствует | отсутствует (sync не завершился) / другой acc |
    | SHOP | `shop-loaded.cart` | стабильно N | 0 после resume (transient clear) |
    | SHOP | `shop-sync.acc` | стабильный X | `null` (accountId провалился) |
    | CATALOG | `catalog-apply.products` | 15 | 0 / меньше / отсутствует (пустой/промежуточный каталог) |
    | UI | `centreTop` / `productHit.top` | элемент карточки/грид | overlay/backdrop/fixed-слой |
    | UI | `centrePointerEvents`/`bodyPointerEvents` | `auto` | `none` |
    | UI | `wheelModal` / `wheelBackdrop` | `false` | `true` (завис оверлей) |
    | UI | `bodyOverflow` | `visible` | `hidden` (застрявший scroll-lock) |
    | UI | `horizontalOverflow` | `false` | `true` |
    | LIFECYCLE | `visibility`/`pageshow` | есть переход hidden→visible | тот же, но без последующей корректной реинициализации |
    | LIFECYCLE | `diag-installed` | 1+ (каждый = полный boot, завершённый) | reinit БЕЗ завершения (нет catalog-apply 15 / shop-loaded) или reload-loop (много подряд) |
    | NETWORK | `online`/`offline` | online в момент tap | `offline` при resume / серия offline↔online |
  - **ТОП-сигналы для BROKEN (смотреть первыми):** (1) `hasSession=false`/`uid`→null/`SIGNED_OUT`/`auth-resolve.outcome=clear`; (2) `nav-redirect{reason}` сразу после `tap` (что откатило маршрут); (3) `tap.topReachesCard=false` + `centreTop`=overlay (`.wheel-backdrop` и т.п.); (4) `catalog-apply.products`≠15 или отсутствует; (5) `shop-loaded` отсутствует или `cart=0`; (6) `pointerEvents=none` / `bodyOverflow=hidden`; (7) отсутствие `route{to:/product}` после tap (navigate не вызвался); (8) серия `diag-installed` без завершённого набора (reload-loop / discard). Прислать полный JSON — сравню построчно по этой таблице и дам конкретный diff (HEALTHY-строка vs BROKEN-строка).

## LAV-BUG-057 — Mobile long-idle: открытый товар мгновенно откатывается на home (PUSH /product → REPLACE /) — inactivity boot-check пере-выполняется на навигации со stale timestamp
- **Module:** `src/hooks/useInactivityRedirect.js` (+ новый чистый helper `src/lib/inactivity.js`)
- **Platform:** mobile (после долгого фона/idle; логика общая)
- **Environment:** Production (реальное устройство — подтверждено ручной инструментацией владельца)
- **Priority:** P1 · **Severity:** S2 · **Status:** FIXED (client-only) — воспроизведено ДО и подтверждено ОТСУТСТВИЕ ПОСЛЕ (playwright-mobile), build+`npm test` зелёные; финальное подтверждение на устройстве за владельцем
- **Found By:** Owner (ручная history-инструментация на production) + Claude Code (reproduction)
- **Found Date:** 2026-08-20 · **Developer:** Claude Code
- **Related:** независимая причина, НЕ покрытая LAV-BUG-052/056 (те — про auth-null). Это баг inactivity-хука.
- **Description:** После долгого idle тап по карточке товара «не открывает» товар: страница товара открывается и **мгновенно откатывается на главную**. Refresh/повторные тапы временно «оживляют».
- **LIVE-доказательство (owner, history-инструментация):** `CLICK CAPTURE href=/product/21, location=/` → `HISTORY PUSH /product/21 FROM /` → **`HISTORY REPLACE / FROM /product/21`** → `CLICK AFTER location=/`. Навигация в товар проходит (PUSH), затем код делает `replace('/')`. НЕ touch/overlay (тап доходит, PUSH успешен).
- **Reproduction (playwright-mobile, production build):** при смонтированном приложении со **stale `lv_last_activity` (≥30мин)** тап по товару → `PUSH /product/20 FROM /` → `REPLACE / FROM /product/20`; `nav-redirect {reason:"idle-30m", from:"/product/20", idleMin:7512}`. Точное совпадение с owner-сигнатурой.
- **Root Cause (доказан):** в `useInactivityRedirect` boot-эффект вызывает `maybeRedirect(bootLastRef.current)` и имел `maybeRedirect` в зависимостях. `maybeRedirect` зависит от `navigate` (из `useNavigate()`), **идентичность которого меняется при каждой смене маршрута** (в этой версии react-router). Поэтому при навигации (PUSH /product) эффект **пере-создавался и ПОВТОРНО вызывал boot-проверку** `maybeRedirect(bootLastRef.current)` со **STALE `bootLastRef`** (захвачен один раз при первом рендере; при монтировании после долгого idle он ≥30мин). Тогда `pathRef='/product/..'≠'/'` + stale≥30мин → `navigate('/', {replace:true})` → REPLACE на home. `route-change`-стамп не спасал: он выполняется в СВОЁМ эффекте после коммита, а boot-re-run стрелял stale `bootLastRef`, а не `lv_last_activity`.
- **Почему 052/056 не поймали:** обе правки — про auth (`AccountHomeRedirect` + `AuthContext` transient-null). Здесь причина иная — effect-deps/stale-ref в inactivity-хуке. В прошлых расследованиях inactivity считался «намеренным 30-мин редиректом»; re-run-on-navigation не был выявлен, т.к. триггер требует, чтобы приложение было **смонтировано** при уже устаревшем `lv_last_activity` (реальный resume-reload после discard), чего 60-цикловый стресс со свежим стартом не создавал.
- **Fix Summary (минимальный, client-only, поведение auth/cart/checkout не тронуто):** boot-эффект теперь выполняется **ровно один раз** (deps `[]`); актуальный `maybeRedirect` вызывается через `maybeRedirectRef` (ref обновляется отдельным эффектом). Слушатели `visibilitychange`/`pageshow` навешиваются один раз и зовут `maybeRedirectRef.current(readLast())` — со **свежим** timestamp. Условие редиректа вынесено в чистый `shouldRedirectHome()` (`src/lib/inactivity.js`) и покрыто unit-тестом. Навигация больше НЕ пере-запускает boot-проверку → stale `bootLastRef` не стреляет. **Намеренное поведение сохранено:** реальный resume после ≥30мин на non-home → home (через listeners + live `readLast()`), и одноразовый cold-boot-on-product-after-idle → home.
- **Fix Verification checklist:**
  - [x] Reproduced ДО фикса: `PUSH /product/20 → REPLACE /` (reason idle-30m). 
  - [x] После фикса: тап по товару со stale bootLast → `PUSH /product/20` только, **REPLACE нет**, route остаётся `/product/20`.
  - [x] Намеренное сохранено: genuine long-idle resume (visibilitychange, stale live) на `/product` → `REPLACE / FROM /product/20` (idle-30m) — по-прежнему работает.
  - [x] `vite build` — успешно; `npm test` — auth 13/13 + inactivity 8/8 (включая «home mount (stale) → tap product → stays on /product (no REPLACE)»).
  - [x] Console 0 errors.
  - [ ] **Real device (за владельцем):** авторизованный, реальный OS-suspend → возврат → тап по товару открывает и НЕ откатывается на home.
- **Regression History:** 2026-08-20 — reproduced+fixed+verified в playwright-mobile (before/after сигнатура), build+tests зелёные. Реальное устройство — за владельцем.
- **Changed files:** `src/hooks/useInactivityRedirect.js`, `src/lib/inactivity.js` (new), `tests/inactivity.test.mjs` (new), `package.json` (`npm test` запускает оба набора).
- **Remaining Risks:** cold-boot-on-product-after-idle всё ещё уводит на home (это существующее намеренное 30-мин поведение, не баг; при желании владельца — отдельная задача). Диагностика (`window.__lavDiagDetailed`) остаётся для контроля.

## LAV-BUG-058 — SECURITY: /admin доступен ЧУЖОМУ аккаунту — is_admin() был захардкожен на неверный email
- **Module:** `public.is_admin()` (`supabase/admin-lockdown.sql`) + `profiles.role` + frontend-гейт (`src/pages/AdminPage.jsx`)
- **Platform:** both (серверная авторизация)
- **Environment:** Production — подтверждено владельцем (вошёл под `alekberov.ceyhun2002@gmail.com` и открыл полноценный /admin)
- **Priority:** P0 · **Severity:** S1 (privilege escalation) · **Status:** FIXED в коде+SQL — **требует применения владельцем** `supabase/fix-admin-owner.sql`
- **Found By:** Owner (report) · **Found Date:** 2026-08-21 · **Developer:** Claude Code
- **Description:** Обычный аккаунт `alekberov.ceyhun2002@gmail.com` проходил серверный `is_admin()` и получал полный доступ к админ-панели. Единственный настоящий владелец — `olegperov2002@gmail.com`.
- **Root Cause (точный):** прежняя `is_admin()` (деплой из `admin-lockdown.sql`) была захардкожена на чужой email: `... and lower(auth.jwt()->>'email') = 'alekberov.ceyhun2002@gmail.com'`, и этому же аккаунту скриптами `admin-lockdown.sql`/`admin-access-email.sql` выставлен `profiles.role='admin'`. Обе части выполнялись → alekberov проходил гейт. Владелец `olegperov2002@gmail.com` НЕ был назначен админом нигде. Frontend `AdminPage.jsx` (`ADMIN_OTP_EMAIL`) тоже указывал на alekberov.
- **Fix Summary (server-side, минимально):** новая идемпотентная миграция `supabase/fix-admin-owner.sql`: (1) находит immutable UUID `olegperov2002@gmail.com` в `auth.users`; (2) снимает `role='admin'` у ВСЕХ, кроме владельца (в т.ч. alekberov); (3) выставляет `role='admin'` владельцу (upsert профиля); (4) пересоздаёт `is_admin()` с **вшитым owner UUID** как основной identity: `auth.uid() = OWNER_UUID AND exists(profiles.role='admin') AND jwt email = olegperov` (тройная защита). Frontend `ADMIN_OTP_EMAIL` → `olegperov2002@gmail.com` (это лишь UX-гейт; серверная is_admin — реальная граница). Старые `admin-lockdown.sql`/`admin-access-email.sql` помечены superseded, email в них исправлен на владельца (чтобы не были footgun). Все admin-only RPC/RLS (admin_list_users, promo, wheel, products, orders, system logs) опираются на исправленную is_admin() — frontend-гейт не сильнее backend.
- **Fix Verification checklist:**
  - [x] Root cause подтверждён кодом: `is_admin()` и оба SQL-скрипта хардкодили alekberov; frontend ADMIN_OTP_EMAIL=alekberov.
  - [x] Frontend: `vite build` OK; в собранном бандле теперь `olegperov2002@gmail.com`, `alekberov*` в бандле ОТСУТСТВУЕТ; `npm test` 21/21.
  - [ ] **OWNER ACTION REQUIRED:** выполнить `supabase/fix-admin-owner.sql` в Supabase → SQL Editor. Затем LIVE: `select u.email,p.role from profiles p join auth.users u on u.id=p.id where p.role='admin'` → ровно одна строка olegperov; под olegperov is_admin()=true (/admin работает); под alekberov is_admin()=false (/admin → 404, admin RPC → AUTH_REQUIRED).
- **Regression History:** 2026-08-21 — код+SQL готовы; frontend build+bundle verified. Серверная часть NOT LIVE до применения SQL (окружение блокирует service_role/прямой SQL-write — sanctioned workflow: владелец запускает файл в SQL Editor, как и все прошлые миграции).
...
- **Remaining Risks:** до применения SQL alekberov сохраняет доступ (серверная is_admin ещё старая). Применить безотлагательно. Если владелец `olegperov2002@gmail.com` ещё ни разу не входил — миграция остановится с понятной ошибкой (пусть войдёт через Google один раз, затем повторить).
