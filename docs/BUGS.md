...
- **Priority:** P1
- **Severity:** S2
- **Status:** FIXED — NEEDS REGRESSION (исправление в рабочем дереве; не закоммичено/не задеплоено; живая регрессия не пройдена)
- **Found By:** Owner (репорт)
- **Found Date:** 2026-08-01
- **Developer:** Claude Code
- **QA:** Pending (владелец) — живая проверка на реальной админ-почте не выполнена
- **Release:** Pre-1.0
...
- **Regression History:** NOT VERIFIED (живая проверка на реальной админ-почте не выполнена; `vite build` — успешно; `git grep` подтвердил отсутствие ожидания 8 цифр в коде).
