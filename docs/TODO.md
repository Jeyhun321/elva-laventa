...
- [ ] **Task 3 — impersonation (владелец, ОБЯЗАТЕЛЬНО применить SQL):** выполнить `supabase/admin-impersonation.sql` (grant-таблица TTL 45м + is_admin-gated RPC start/end + read/write cart/favorites/profile/wheel/orders + аудит). До применения кнопка «Войти как пользователь» вернёт 404/ошибку. Код (frontend) уже в проде. После — прогнать manual security-тесты (owner→User A real data; reversible write на реальный UUID; A/B без утечки; non-owner из DevTools → 403; place_order в имперсонации заблокирован; обычный checkout не тронут). (F-014)
