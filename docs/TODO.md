...
- [ ] **Закупки Архив/Удаление (владелец, ОБЯЗАТЕЛЬНО):** выполнить `supabase/procurement-archive-delete.sql` (после `procurement-module.sql` + `procurement-product-flow.sql`) — иначе физическое **Удаление** недоступно (кнопка вернёт «выполните procurement-archive-delete.sql»); архив/восстановление продолжат работать через fallback, но без серверного аудита. Аддитивно: `procurements.archived_at/archived_by`, RPC `archive_/restore_/delete_procurement` (SECURITY DEFINER + is_admin, аудит), `procurement_analytics` включает архивные закупки. Проверка после Run: `archive_procurement(uuid)` → в архив; `restore_procurement(uuid)` → назад; `delete_procurement(<без товара>)` → удалит; закупка со связанным `product_id` → `delete_procurement` = `PROCUREMENT_LINKED`; обычный/anon → `AUTH_REQUIRED`. (см. FEATURES #F-019)
- [ ] **Procurement-first flow (владелец, ОБЯЗАТЕЛЬНО):** выполнить `supabase/procurement-product-flow.sql` (после `procurement-module.sql`) — иначе фото/варианты закупки, кнопка «Добавить в Товары» и обновлённая аналитика не работают (форма/панель покажут подсказку). Аддитивно: `procurements.images/variants/promoted_to_product_at/stock_applied`, `planned_sale_unit_price`→nullable, `promote_procurement_to_product`, обновлённый `procurement_analytics`. Проверка после Run: создать закупку с фото+вариантами → «Добавить в Товары» → в «Товары» появился ЧЕРНОВИК (is_active=false, price 0, есть фото/цвета/размеры); повторный клик/двойной клик дубль НЕ создаёт; закуп. цена в товар не попала. (см. FEATURES #F-018, DECISIONS #D-010)
- [ ] **Sales Analytics — отдельный будущий модуль:** реальные продажи/выручка/заказы/средний чек из order-subsystem. НЕ смешивать с Procurement Analytics. (D-010)
- [ ] **Combined Profit Analytics — после Sales Analytics:** сопоставление закупочной себестоимости и фактической выручки → реальная прибыль бизнеса. (D-010)
- [ ] **Числовой склад товара (future):** сейчас `products.in_stock` — булев; перенос закупки ставит `in_stock=true` без числового инкремента. Рассмотреть per-variant числовой inventory с защитой от double-add (флаг `stock_applied` уже заложен). (D-010)
- [ ] **Закупки / Поставщики / Аналитика (владелец, ОБЯЗАТЕЛЬНО):** выполнить `supabase/procurement-module.sql` — иначе модули Закупки/Поставщики/Аналитика в админке покажут подсказку «выполните procurement-module.sql» (таблиц/RPC ещё нет). Миграция аддитивна и идемпотентна: создаёт `suppliers`, `supplier_points`, `procurements` (+ GENERATED суммы/прибыль/маржа/остаток), RLS только `is_admin()`, RPC `procurement_analytics`, триггер проверки принадлежности точки поставщику. Проверка после Run: под owner `insert into suppliers(name) values('Тест')`; `select public.procurement_analytics(current_date-30, current_date)`; под обычным пользователем `select * from procurements` → пусто/denied. Sidebar-навигация и UI уже задеплоены. (см. FEATURES #F-017, DECISIONS #D-009). **Живой прогон (owner-сессия):** добавить закупку (qty10, закуп20, продажа35 → сумма200/выручка350/прибыль150/маржа42.86%), поставщика A с точками A1/A2 (в форме показываются только они), проверить сумму аналитики против записей.
