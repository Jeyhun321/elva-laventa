...
- **Упрощение auth (2026-08-21):** удалена вторая admin-OTP-система (экран «Подтвердите вход»/«Отправить код», `EmailOtpScreen`, `AdminSetupRequired`, `ADMIN_OTP_EMAIL`/`isOwnerEmail`, `signInWithOtp`/`verifyOtp`, sessionStorage-ключ `elva-admin-otp-verified` и все флаги `otpVerified`/OTP-cooldown/resend). Теперь единый source of truth: текущая Supabase-сессия + серверная `is_admin()`. Гейт `/admin`: anon → обычный вход (Google/пароль); authenticated → фронт спрашивает сервер `adminSupabase.rpc('is_admin')` → true → админка сразу (без второго OTP), false → обычный 404. Смена аккаунта: `onAuthStateChange` → пересчёт `is_admin()` → мгновенный unmount панели/очистка; возврат на owner → админка без OTP. Никаких client-side admin-unlock флагов не осталось. LIVE: anon `is_admin()`→false (сервер отклоняет).
