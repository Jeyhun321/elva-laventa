...
## [F-019] Закупки — Архив / Восстановление / Безопасное удаление
- **Описание:** у каждой закупки добавлено самостоятельное действие **Удалить** рядом с существующими Изм./Добавить в Товары/Архив, и приведён в порядок жизненный цикл записи. Действия строки собраны в компактное overflow-меню **⋯** (не перегружает таблицу, удобно на мобильном).
- **Архив (soft-delete):** запись НЕ удаляется из БД — помечается `archived=true` (+ `archived_at`/`archived_by`), исчезает из активного списка, но полностью сохраняет историю (поставщик, SKU, фото, варианты, закуп. цену, количество, связь с товаром, timestamps).
- **Просмотр архива:** фильтр-переключатель **Активные / Архив / Все** со счётчиками (по умолчанию — Активные). У архивной строки — заметный badge **«В архиве»**; в архиве действие «Добавить в Товары» скрыто (сначала Восстановить).
- **Восстановление:** действие **Восстановить** → `archived=false`, запись снова в Активных, связи/данные не пересоздаются (тот же UUID).
- **Удаление (hard delete):** отдельное destructive-действие с подтверждающей модалкой проекта (не браузерный `confirm()`). **Сервер — source of truth**: RPC сам проверяет зависимости. Несвязанную закупку (`product_id IS NULL`) owner удаляет физически. Связанную с товаром сервер отклоняет (`PROCUREMENT_LINKED`) → UI показывает «Эта закупка уже связана с товаром и не может быть удалена. Переместите её в архив.» и предлагает кнопку «В архив». Аудит пишется в той же транзакции ДО удаления.
- **Аналитика — историчность:** `procurement_analytics` теперь ВКЛЮЧАЕТ архивированные закупки (архив — UI-lifecycle, а не отмена факта закупки), поэтому исторические суммы не обнуляются при архивации. Из истории исчезают только реально удалённые записи. Активный список и его счётчик считают только `archived=false`.
- **Безопасность/аудит:** `archive_/restore_/delete_procurement` — SECURITY DEFINER + `public.is_admin()` (owner-only; обычный/anon → `AUTH_REQUIRED`). Frontend `deleteProcurement` идёт только через RPC (не «тихий» прямой delete); при неприменённой миграции — понятная ошибка (`DELETE_RPC_MISSING`), архив/восстановление имеют безопасный fallback на прямой update. Логи: `PROCUREMENT_ARCHIVED` / `PROCUREMENT_RESTORED` / `PROCUREMENT_DELETED` (owner UUID, procurement id, SKU, timestamp; без секретов).
- **Дата:** 2026-08-22
- **Изменённые файлы:** `supabase/procurement-archive-delete.sql` (new), `src/admin/procurement.js` (listProcurements archived-filter, restore/deleteProcurement, RPC-first archive), `src/components/admin/ProcurementPanel.jsx` (view-фильтр, RowMenu ⋯, badge, delete-модалка, restore), `src/components/admin/AnalyticsPanel.jsx` + `SuppliersPanel.jsx` (archived:'all' для истории), `src/styles/index.css` (badge/menu/danger/confirm).
- **UI-доработка (2026-08-22):** меню действий строки (⋯) переведено на **портал** (`createPortal` → `document.body`, `position:fixed`), чтобы не обрезаться overflow таблицы; позицию считает `src/lib/popover.js` (`popoverPosition`: правое выравнивание, авто-флип вверх при нехватке места снизу, клэмп по краям viewport, пересчёт на scroll/resize). Закрытие — клик вне / Esc / выбор действия; «Удалить» — последний, danger-стиль. Чисто UI, backend не тронут. Тесты: `tests/popover.test.mjs` (7 кейсов).
- **OWNER ACTION REQUIRED:** применить `supabase/procurement-archive-delete.sql` (после `procurement-module.sql` + `procurement-product-flow.sql`).
